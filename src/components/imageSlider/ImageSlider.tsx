...
const images: ImageData[] = [
  {
    ...
    description: "Indulge in the exquisite taste of our premium coffee. From its captivating aroma to its rich flavor of, each cup is a masterpiece.",
  },
  {
    src: image2,
    header1: "Start your day with a boost,",
    header2: "enjoy our coffee",
    description: "Not only delicious, our coffee is also packed with antioxidants, boosting your health and energy throught the day.",
  ...
  },
  {
    ...
    description: "Sourced from the finest plantations and processed using traditional methods, our coffee is crafted to perfection.",
  },
];
    ...
    <section className="sm:p-8 md:p-20 w-full bg-calm-brown">
      ...
      >
        <div className="description flex flex-col text-black text-center md:text-left lg:w-1/2 sm:w-full p-4 md:p-0">
          <h1 className="font-black text-4xl sm:text-5xl md:text-6xl">{images[currentIndex].header1}</h1>
          <h2 className="font-black text-2xl sm:text-3xl md:text-4xl my-4">{images[currentIndex].header2}</h2>
          <p className="font-extralight text-xl sm:text-xl md:text-lg">{images[currentIndex].description}</p>
        </div>
        <Image
        ...
        {images.map((_, index) => (
          <div
            ...
            className={`h-1 w-6 sm:w-8 md:w-10 mx-1 ${
              index === currentIndex ? "bg-white rounded-xl" : "bg-black rounded-xl"
